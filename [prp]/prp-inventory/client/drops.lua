...
function GetDrops()
    QBCore.Functions.TriggerCallback('qb-inventory:server:GetCurrentDrops', function(drops)
        ...
        for k, v in pairs(drops) do
            if v.coords then
                TriggerEvent('qb-inventory:client:createDrop', k, v.coords)
            end
        end
    end)
end

function GetClosestDrop(maxDistance)
    ...
    for dropId, dropData in pairs(Drops) do
        ...
        if dropData.entity and DoesEntityExist(dropData.entity) then
            coords = GetEntityCoords(dropData.entity)
        end
    ...
    return closestDrop, closestDistance
end
...
RegisterNetEvent('qb-inventory:client:removeDropTarget', function(dropId)
    if type(dropId) == 'string' then
        TriggerEvent('qb-inventory:client:removeDropObject', dropId)
        return
    end
