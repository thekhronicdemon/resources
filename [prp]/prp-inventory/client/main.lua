...
local inventoryFrontendPed = nil
local inventoryFrontendActive = false
local inventoryPlayerHidden = false
local inventoryPlayerHideThread = false

local function SetInventoryPlayerHidden(hidden)
    inventoryPlayerHidden = hidden == true

    if inventoryPlayerHidden and not inventoryPlayerHideThread then
        inventoryPlayerHideThread = true
        CreateThread(function()
            while inventoryPlayerHidden do
            ...
            inventoryPlayerHideThread = false
        end)
    end
end

local function StopFrontendPedPreview()
    inventoryFrontendActive = false
    Citizen.InvokeNative(0x5E62BE5DC58E9E06)
    SetFrontendActive(false)

    if inventoryFrontendPed and DoesEntityExist(inventoryFrontendPed) then
        DeleteEntity(inventoryFrontendPed)
    end

    inventoryFrontendPed = nil

    SendNUIMessage({
        action = 'characterPreview',
        active = false
    })
end

local function StopInventoryPreview()
    SetInventoryPlayerHidden(false)
    StopFrontendPedPreview()
end

local function StartFrontendPedPreview()
    StopFrontendPedPreview()
    return false
end
...
local function FormatWeaponAttachments(itemdata)
    if not itemdata or type(itemdata.info) ~= 'table' then
        return {}
    end

    local attachments = {}
    local weaponName = itemdata.name
    local tintIndex = tonumber(itemdata.info.tint)
    if tintIndex then
        local tintItem = itemdata.info.tintItem or ((IsMk2WeaponName(weaponName) and 'weapontint_mk2_' or 'weapontint_') .. tintIndex)
        local itemInfo = QBCore.Shared.Items[tintItem]
        attachments[#attachments + 1] = {
            attachment = tintItem,
            tint = tintIndex,
            isTint = true,
            label = itemInfo and itemInfo.label or ('Weapon Tint ' .. tintIndex),
            image = itemInfo and itemInfo.image or 'weapontint_black.png'
        }
    end

    if not itemdata.info.attachments or #itemdata.info.attachments == 0 then return attachments end

    local WeaponAttachments = exports['qb-weapons']:getConfigWeaponAttachments()
    if not WeaponAttachments then return attachments end
...
local function HasWeaponTint(itemdata, tintIndex)
    return itemdata and type(itemdata.info) == 'table' and tonumber(itemdata.info.tint) == tonumber(tintIndex)
end
...
local function FormatAvailableWeaponAttachments(itemdata)
    ...
    for _, invItem in pairs(PlayerData.items) do
        if invItem and invItem.name then
            local tintIndex, mk2Tint = ParseWeaponTintItem(invItem.name)
            if tintIndex and (mk2Tint == IsMk2WeaponName(itemdata.name)) and not HasWeaponTint(itemdata, tintIndex) then
                ...
                available[#available + 1] = {
                    ...
                    image = itemInfo and itemInfo.image or invItem.image or 'weapontint_black.png',
                    amount = invItem.amount or 1,
                    slot = invItem.slot
                }
            elseif WeaponAttachments[invItem.name] then
...
local function FormatDeviceAttachments(itemdata)
    ...
    if PhoneDeviceItems[itemdata.name] and info.simNumber then
...
local function FormatAvailableDeviceAttachments(itemdata)
    ...
    local info = type(itemdata.info) == 'table' and itemdata.info or {}
    if PhoneDeviceItems[itemdata.name] and info.simNumber then return available end
...
local function BuildDeviceAttachmentData(itemdata, message)
    ...
    if not note then
        if itemdata and PhoneDeviceItems[itemdata.name] then
...
local function AddAvailableDeviceAttachment(payload, deviceData, invItem)
    ...
    if PhoneDeviceItems[deviceData.name] and info.simNumber then return end
...
local function GetEquippedItem(slot)
    if not PlayerData or type(PlayerData.metadata) ~= 'table' then return nil end
    local equipment = PlayerData.metadata.equipment
...
function HasItem(items, amount)
    ...
    local function checkCollection(collection)
        if type(collection) ~= 'table' then return false end

        for _, itemData in pairs(collection) do
            local itemAmount = tonumber(itemData and itemData.amount) or 1
            if isTable then
                for k, v in pairs(items) do
                    if itemData and itemData.name == (isArray and v or k) and ((amount and itemAmount >= amount) or (not isArray and itemAmount >= v) or (not amount and isArray)) then
                        count = count + 1
            ...
            elseif itemData and itemData.name == items and (not amount or itemAmount >= amount) then
                return true
            end
        end
    ...
    if PlayerData then
        if checkCollection(PlayerData.items) then return true end

        local metadata = PlayerData.metadata or {}
        if checkCollection(metadata.equipment) then return true end
    end
...
RegisterNetEvent('prp-inventory:client:equipParachute', function()
    local ped = PlayerPedId()
    local parachuteHash = GetHashKey('GADGET_PARACHUTE')
    GiveWeaponToPed(ped, parachuteHash, 1, false, true)
    SetPedGadget(ped, parachuteHash, true)
end)

RegisterNetEvent('prp-inventory:client:removeParachute', function()
...
RegisterNetEvent('qb-inventory:client:updateInventory', function(serverItems)
    ...
    if type(serverItems) == 'table' and (serverItems.other or serverItems.backpack) then
        if PlayerData and serverItems.inventory then
            PlayerData.items = serverItems.inventory
        end

        SendNUIMessage({
            action = 'update',
            inventory = serverItems.inventory or (PlayerData and PlayerData.items) or {},
            other = serverItems.other,
            backpack = serverItems.backpack,
            equipment = serverItems.equipment or (PlayerData and PlayerData.metadata and PlayerData.metadata.equipment) or nil,
            quickslots = serverItems.quickslots or (PlayerData and PlayerData.metadata and PlayerData.metadata.quickslots) or nil
        })
        return
    end
...
local function OpenInventoryNui(items, other, playerMetadata, characterPreview)
    SetInventoryPlayerHidden(false)
...
RegisterNetEvent('qb-inventory:client:openInventory', function(items, other, metadata)
    ...
    hotbarShown = false
    local characterPreview = StartFrontendPedPreview()
    OpenInventoryNui(items, other, playerMetadata, characterPreview)
end)
...
RegisterNUICallback('Notify', function(data, cb)
    local message = data and (data.message or data.text) or 'Action failed'
    local notifyType = data and data.type or 'primary'
    local length = data and data.length or 3500
    local icon = data and data.icon or nil
    QBCore.Functions.Notify(message, notifyType, length, icon)
    cb('ok')
end)
...
RegisterNUICallback('GetCraftingData', function(_, cb)
    QBCore.Functions.TriggerCallback('prp-inventory:server:getCraftingData', function(result)
        cb(result or { success = false, recipes = {} })
    end)
end)

RegisterNUICallback('CraftGridRecipe', function(data, cb)
    if not data or not data.recipeId then
        cb({ success = false, message = 'No Craft Found' })
        return
    end

    local function finishCrafting()
        QBCore.Functions.TriggerCallback('prp-inventory:server:craftGridRecipe', function(result)
            if result and result.inventory then
                PlayerData = QBCore.Functions.GetPlayerData()
                if PlayerData then PlayerData.items = result.inventory end
                SendNUIMessage({
                    action = 'update',
                    inventory = result.inventory
                })
            end
            cb(result or { success = false, message = 'Crafting failed.' })
        end, data)
    end

    QBCore.Functions.Progressbar('prp_inventory_crafting', 'Crafting item...', 60000, false, true, {
...
RegisterNUICallback('OpenBackpack', function(_, cb)
    TriggerServerEvent('prp-inventory:server:openBackpack')
    cb({ success = true })
end)
...
RegisterNUICallback('UnequipItem', function(data, cb)
    QBCore.Functions.TriggerCallback('qb-inventory:server:unequipItem', function(result)
        ...
        cb(result or { success = false })
    end, data.equipmentSlot, data.targetSlot)
end)
...
RegisterNUICallback('SetQuickSlot', function(data, cb)
    local ok, result = pcall(function()
        ...
        local quickSlot = tonumber(data.quickSlot)
        local itemSlot = tonumber(data.itemSlot)
        ...
        if not PlayerData then
            return { success = false, message = 'Player data unavailable' }
        end

        PlayerData.items = type(PlayerData.items) == 'table' and PlayerData.items or {}
        PlayerData.metadata = type(PlayerData.metadata) == 'table' and PlayerData.metadata or {}

        local quickslots = type(PlayerData.metadata.quickslots) == 'table' and PlayerData.metadata.quickslots or {}
        quickslots[quickSlot] = nil
        quickslots[tostring(quickSlot)] = nil

        if itemSlot then
            local item = GetPlayerItemBySlot(itemSlot)
            if not item then
        ...
        PlayerData.metadata.quickslots = quickslots
        TriggerServerEvent('qb-inventory:server:setQuickSlotDirect', quickSlot, itemSlot)

        return {
            success = true,
            quickslots = quickslots,
            inventory = PlayerData.items,
            equipment = PlayerData.metadata.equipment or {}
        }
    end)
    ...
    if ok then
        ...
        print(('[prp-inventory] SetQuickSlot NUI error: %s'):format(result))
        cb({ success = false, message = 'Quick slot failed' })
    end
end)
...
RegisterNUICallback('ApplyAttachment', function(data, cb)
    ...
    QBCore.Functions.TriggerCallback('qb-inventory:server:applyWeaponAttachment', function(result)
        if result and result.success and result.WeaponData then
            if GetSelectedPedWeapon(ped) == joaat(result.WeaponData.name) and result.isTint and result.tint then
                SetPedWeaponTintIndex(ped, joaat(result.WeaponData.name), result.tint)
            end
...
RegisterNUICallback('RemoveAttachment', function(data, cb)
    ...
    if not WeaponData or not data.AttachmentData then
        cb({ success = false, message = 'No weapon mod selected' })
        return
    end

    local Attachment = nil
    if not data.AttachmentData.isTint then
        if not allAttachments or not allAttachments[data.AttachmentData.attachment] then
            cb({ success = false, message = 'No weapon mod selected' })
            return
        end
        Attachment = allAttachments[data.AttachmentData.attachment][WeaponData.name]
    end

    if not data.AttachmentData.isTint and not Attachment then
    ...
    QBCore.Functions.TriggerCallback('qb-inventory:server:removeWeaponAttachment', function(result)
        if result and result.success and result.WeaponData then
            if GetSelectedPedWeapon(ped) == joaat(WeaponData.name) and result.isTint then
                SetPedWeaponTintIndex(ped, joaat(WeaponData.name), 0)
            elseif GetSelectedPedWeapon(ped) == joaat(WeaponData.name) and Attachment then
