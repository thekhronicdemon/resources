...
local function GetFirstFreeSlot(items, maxSlots)
    ...
    for i = 1, maxSlots do
        local occupied = items[i] ~= nil or items[tostring(i)] ~= nil

        if not occupied then
            for _, item in pairs(items) do
                if item and tonumber(item.slot) == i then
                    occupied = true
                    break
                end
            end
...
local function GenerateSimCardNumber()
    local number = '04'
    for _ = 1, 8 do
...
local function GenerateUniqueSimCardNumber()
    for _ = 1, 25 do
        local number = GenerateSimCardNumber()
    ...
    return GenerateSimCardNumber()
end

local function NormalizeSimCardInfo(info)
    info = type(info) == 'table' and info or {}
    info.simNumber = info.simNumber or GenerateUniqueSimCardNumber()
...
local function NormalizeInventorySimCards(items)
    local changed = false
    items = items or {}
    ...
    for key, item in pairs(items) do
        if item and item.name and item.name:lower() == 'simcard' then
            local beforeNumber = type(item.info) == 'table' and item.info.simNumber or nil
            local beforeSerial = type(item.info) == 'table' and item.info.simSerial or nil
            local beforeDescription = item.description
            item.info = NormalizeSimCardInfo(item.info)
            item.description = item.info.description
            if installedSimNumbers[tostring(item.info.simNumber)] then
                items[key] = nil
                changed = true
            else
                changed = changed or not beforeNumber or not beforeSerial or beforeDescription ~= item.description
            end
        elseif item and item.name and (item.name:lower() == 'crypto_usb' or item.name:lower() == 'cryptostick') then
            local info = type(item.info) == 'table' and item.info or {}
        ...
        for _, item in pairs(shopItems) do
            ...
            if itemInfo then
                items[slot] = {
                    ...
                    slot = slot,
                    rarity = (item.info and item.info.rarity) or itemInfo.rarity or 'common',
...
function LoadInventory(source, citizenid)
    ...
    for _, item in pairs(inventory) do
        if item then
            ...
            if itemInfo then
                item.info = itemInfo['name'] == 'simcard' and NormalizeSimCardInfo(item.info) or (item.info or {})
...
function HasItem(source, items, amount)
    ...
    if checkCollection(Player.PlayerData.items) then return true end
...
function OpenInventoryById(source, targetId)
    ...
    if NormalizeInventorySimCards(QBPlayer.PlayerData.items) then
        QBPlayer.Functions.SetPlayerData('items', QBPlayer.PlayerData.items)
    end
    if SyncEquippedArmorFromPed then SyncEquippedArmorFromPed(QBPlayer) end
    if NormalizeInventorySimCards(TargetPlayer.PlayerData.items) then
        TargetPlayer.Functions.SetPlayerData('items', TargetPlayer.PlayerData.items)
    end
    ...
    TriggerClientEvent('qb-inventory:client:openInventory', source, playerItems, formattedInventory, QBPlayer.PlayerData.metadata)
end
...
function OpenShop(source, name)
    ...
    if NormalizeInventorySimCards(Player.PlayerData.items) then
        Player.Functions.SetPlayerData('items', Player.PlayerData.items)
    end
    ...
    local formattedInventory = {
        ...
        sellBackRate = Config.ShopSellBackRate or 0.70
    }
    TriggerClientEvent('qb-inventory:client:openInventory', source, Player.PlayerData.items, formattedInventory, Player.PlayerData.metadata)
end
...
function OpenInventory(source, identifier, data)
    ...
    if not identifier then
        local backpackInventory = GetEquippedBackpackInventoryPayload and GetEquippedBackpackInventoryPayload(QBPlayer) or nil
        Player(source).state.inv_busy = true
        TriggerClientEvent('qb-inventory:client:openInventory', source, QBPlayer.PlayerData.items, backpackInventory, QBPlayer.PlayerData.metadata)
        return
    end
    ...
    TriggerClientEvent('qb-inventory:client:openInventory', source, QBPlayer.PlayerData.items, formattedInventory, QBPlayer.PlayerData.metadata)
end
...
function AddItem(identifier, item, amount, slot, info, reason)
    ...
    if not updated then
        ...
        inventory[slot] = {
            ...
            slot = slot,
            combinable = itemInfo.combinable,
            rarity = ((info or {}).rarity) or itemInfo.rarity or 'common',
            armor = itemInfo.armor == true,
            backpack = itemInfo.backpack == true,
            gadget = itemInfo.gadget == true,
            phone = itemInfo.phone == true
...
function RemoveItem(identifier, item, amount, slot, reason)
    ...
    for key, invItem in pairs(inventory) do
        if invItem and tonumber(invItem.slot) == slot then
    ...
    amount = tonumber(amount) or 1
    inventoryItem.amount = tonumber(inventoryItem.amount) or 0
