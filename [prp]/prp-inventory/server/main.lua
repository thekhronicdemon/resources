...
local ArmorPlateCapacity = 20
local ArmorPlateValue = 5
local SaveSingleInventory
local IsBackpackInventoryId

local SetActivePhoneNumber
local ClearActivePhoneNumber
...
local function EnsureSimCardInfo(info)
    info = CloneTable(info)
    local simNumber = info.simNumber or info.phoneNumber or info.number
    if simNumber and tostring(simNumber) ~= '' and tostring(simNumber):lower() ~= 'unknown' then
...
local function GetItemInfo(item)
    ...
    return QBCore.Shared.Items[tostring(item.name):lower()]
end

local function ItemHasFlag(item, flag)
    local itemInfo = GetItemInfo(item)
    return itemInfo and itemInfo[flag] == true
end

local function IsEquipmentItemForSlot(item, equipmentSlot)
    if not item or not item.name then return false end

    local itemName = tostring(item.name):lower()

    -- SIM cards must only go into the SIM slot, never the phone slot.
    if itemName == 'simcard' or itemName == 'sim_card' then
        return equipmentSlot == 'simcard'
    end

    if equipmentSlot == 'phone' then
        return ItemHasFlag(item, 'phone') or PhoneItems[itemName] == true
    elseif equipmentSlot == 'simcard' then
        return ItemHasFlag(item, 'simcard') or SimCardItems[itemName] == true
    elseif equipmentSlot == 'backpack' then
        return ItemHasFlag(item, 'backpack') or BackpackItems[itemName] == true
    elseif equipmentSlot == 'armour' then
        return ItemHasFlag(item, 'armor') or ArmourItems[itemName] == true
    elseif equipmentSlot == 'gadget' or equipmentSlot == 'gadget2' then
        return ItemHasFlag(item, 'gadget') or GadgetItems[itemName] == true
    end
...
local function InitializeEmbeddedInventory(inventoryId, data)
...
local function LoadStoredInventoryItems(identifier)
    if not identifier then return {} end

    local ok, rows = pcall(function()
        return MySQL.query.await('SELECT items FROM inventories WHERE identifier = ? LIMIT 1', { identifier })
    end)
    if not ok then return {} end

    local row = (rows or {})[1]
    if not row or not row.items or row.items == '' then return {} end
...
local function GetOrCreatePersistentInventory(identifier, data)
    local inventory = Inventories[identifier]
    if not inventory then
        inventory = InitializeEmbeddedInventory(identifier, data)
        inventory.items = LoadStoredInventoryItems(identifier)
    end
...
local PlayerBackpackStores = {}

local function DecodeBackpackStore(raw)
...
local function GetPlayerBackpackStoreByCitizenId(citizenid, forceLoad)
    if not citizenid then return {} end

    if not forceLoad and PlayerBackpackStores[citizenid] then
        return PlayerBackpackStores[citizenid]
    end

    local ok, rows = pcall(function()
        return MySQL.query.await('SELECT backpack FROM players WHERE citizenid = ? LIMIT 1', { citizenid })
    end)

    if not ok then
        print(('[prp-inventory] Could not load players.backpack for %s: %s'):format(citizenid, tostring(rows)))
        PlayerBackpackStores[citizenid] = {}
        return PlayerBackpackStores[citizenid]
    end

    local row = rows and rows[1]
    PlayerBackpackStores[citizenid] = DecodeBackpackStore(row and row.backpack)
    return PlayerBackpackStores[citizenid]
end

local function GetPlayerBackpackStore(Player, forceLoad)
    local citizenid = Player and Player.PlayerData and Player.PlayerData.citizenid
    return GetPlayerBackpackStoreByCitizenId(citizenid, forceLoad)
end

local function LooksLikeInventoryItems(value)
...
local function PackInventoryItemsForStorage(items)
    local packed = {}

    for _, item in pairs(items or {}) do
        if item and item.name and item.slot then
            packed[#packed + 1] = CloneTable(item)
        end
    end
...
local function GetStoredBackpackItems(Player, storageId)
    local store = GetPlayerBackpackStore(Player)
    local stored = storageId and store[storageId]

    if type(stored) == 'table' then
        if type(stored.items) == 'table' then return stored.items end
        return stored
    end

    -- Supports the first version of the column if it was saved as a raw inventory table.
    if LooksLikeInventoryItems(store) then return store end

    return {}
end

local function SaveBackpackItemsByCitizenId(citizenid, storageId, items)
    if not citizenid or not storageId then return false end

    local store = GetPlayerBackpackStoreByCitizenId(citizenid)
    if LooksLikeInventoryItems(store) then
        store = {
            [storageId] = CloneTable(store)
        }
    end

    store[storageId] = PackInventoryItemsForStorage(items)
    PlayerBackpackStores[citizenid] = store

    local ok, result = pcall(function()
        return MySQL.update.await('UPDATE players SET backpack = ? WHERE citizenid = ?', {
            json.encode(store),
            citizenid
        })
    end)
    ...
    if not ok then
        print(('[prp-inventory] Could not save players.backpack for %s: %s'):format(citizenid, tostring(result)))
        return false
    end
...
local function SavePlayerBackpackItems(Player, storageId, items)
    local citizenid = Player and Player.PlayerData and Player.PlayerData.citizenid
    return SaveBackpackItemsByCitizenId(citizenid, storageId, items)
end

local function GetOrCreatePlayerBackpackInventory(storageId, data, Player)
    local inventory = Inventories[storageId]
    if not inventory then
        inventory = InitializeEmbeddedInventory(storageId, data)
    end

    inventory.identifier = storageId
    local citizenid = Player and Player.PlayerData and Player.PlayerData.citizenid
    if citizenid and inventory.ownerCitizenid ~= citizenid then
        inventory.items = GetStoredBackpackItems(Player, storageId)
        inventory.ownerCitizenid = citizenid
    end

    inventory.storageKind = 'player_backpack'
    inventory.maxweight = (data and data.maxweight) or inventory.maxweight or Config.StashSize.maxweight
    inventory.slots = (data and data.slots) or inventory.slots or Config.StashSize.slots
    inventory.label = (data and data.label) or inventory.label or storageId
...
local function GetPlayerEquipmentItem(Player, slot, equipmentSlot)
    if not Player or type(Player.PlayerData.items) ~= 'table' then return nil end

    slot = tonumber(slot)
    if slot then
        local directItem = Player.PlayerData.items[slot] or Player.PlayerData.items[tostring(slot)]
        if directItem and IsEquipmentItemForSlot(directItem, equipmentSlot) then
            return directItem
        end
    end

    for _, item in pairs(Player.PlayerData.items) do
        if item and IsEquipmentItemForSlot(item, equipmentSlot) then
            return item
        end
    end
...
local function EnsureEquippedBackpackInfo(Player)
    if not Player then return nil, nil end

    local equipment = GetEquipment(Player)
    local backpack = equipment and equipment.backpack
    if not backpack or not backpack.name then return nil, nil end

    local info = CloneTable(backpack.info)
    local changed = false

    if not info.storageId or tostring(info.storageId) == '' then
        info.storageId = ('backpack_%s_%s%s'):format(Player.PlayerData.citizenid, QBCore.Shared.RandomStr(3), QBCore.Shared.RandomInt(4))
        changed = true
    end

    local slots = tonumber(info.slots) or tonumber(Config.BackpackStorage and Config.BackpackStorage.slots) or 20
    local maxweight = tonumber(info.maxweight or info.maxWeight) or tonumber(Config.BackpackStorage and Config.BackpackStorage.maxweight) or 50000

    if tonumber(info.slots) ~= slots then
        info.slots = slots
        changed = true
    end

    if tonumber(info.maxweight or info.maxWeight) ~= maxweight then
    ...
    if changed then
        backpack.info = info
        backpack.description = ('Backpack storage: %s slots / %s weight'):format(info.slots, info.maxweight)
...
local function ResolveBackpackInventoryAlias(Player, inventoryId)
    if inventoryId == nil then return nil end

    if inventoryId ~= nil and inventoryId ~= '' and inventoryId ~= 'other' and inventoryId ~= 'backpack' then
        return inventoryId
    end

    local _, info = EnsureEquippedBackpackInfo(Player)
    return (info and info.storageId) or inventoryId
end
...
local function FormatEquipmentItem(item, equipmentSlot, Player)
    if not item or not item.name then return nil end
    ...
    if equipmentSlot == 'armour' then
        local plates = tonumber(info.plates)
        if not plates then
            local armor = tonumber(info.armor)
            plates = armor and math.ceil(Clamp(armor, 0, 100) / ArmorPlateValue) or ArmorPlateCapacity
        end
        info.plateCapacity = ArmorPlateCapacity
        info.plates = Clamp(math.floor(plates), 0, ArmorPlateCapacity)
        info.armor = Clamp(info.armor or (info.plates * ArmorPlateValue), 0, 100)
        info.maxArmor = 100
        info.description = ('Armor: %d%% (%d/%d plates)'):format(info.armor, info.plates, ArmorPlateCapacity)
    elseif equipmentSlot == 'backpack' then
        local citizenid = Player and Player.PlayerData and Player.PlayerData.citizenid or 'unknown'
        info.storageId = info.storageId or ('backpack_%s_%s%s'):format(citizenid, QBCore.Shared.RandomStr(3), QBCore.Shared.RandomInt(4))
        info.slots = tonumber(info.slots) or tonumber(Config.BackpackStorage and Config.BackpackStorage.slots) or 20
        info.maxweight = tonumber(info.maxweight or info.maxWeight) or tonumber(Config.BackpackStorage and Config.BackpackStorage.maxweight) or 50000
        info.description = ('Backpack storage: %s slots / %s weight'):format(info.slots, info.maxweight)
    elseif equipmentSlot == 'gadget' and itemName == 'parachute' then
        info.description = 'Equipped parachute gadget'
    elseif equipmentSlot == 'phone' then
        info.description = 'Equipped phone device.'
    elseif equipmentSlot == 'simcard' then
        info = EnsureSimCardInfo(info)
        info.description = ('SIM number: %s'):format(info.simNumber or 'Pending activation')
    elseif equipmentSlot == 'gadget2' then
        info.description = info.description or 'Equipped secondary gadget'
    end
    ...
    return {
        ...
        armor = itemInfo.armor == true,
        backpack = itemInfo.backpack == true,
        gadget = itemInfo.gadget == true,
        gadget2 = itemInfo.gadget == true,
        phone = itemInfo.phone == true,
        simcard = itemInfo.simcard == true or itemName == 'simcard' or itemName == 'sim_card',
    }
end
...
function SyncEquippedArmorFromPed(Player)
    ...
    if currentArmor <= 0 and storedArmor > 0 then
        equipment.armour = nil
        SetEquipment(Player, equipment)
    ...
    info.maxArmor = 100
    info.plateCapacity = ArmorPlateCapacity
    info.plates = Clamp(math.ceil(currentArmor / ArmorPlateValue), 0, ArmorPlateCapacity)
    info.description = ('Armor: %d%% (%d/%d plates)'):format(currentArmor, info.plates, ArmorPlateCapacity)
    armour.info = info
...
local function EquipEquipmentItem(src, equipmentSlot, fromSlot)
    ...
    if equipment[equipmentSlot] then
        return { success = false, message = ('Take off your current %s first'):format(EquipmentLabels[equipmentSlot] or equipmentSlot) }
    end

    local item = GetPlayerEquipmentItem(Player, fromSlot, equipmentSlot)
    if not item then
        return { success = false, message = ('No %s item found'):format(EquipmentLabels[equipmentSlot] or equipmentSlot) }
    end

    local itemSlot = tonumber(item.slot)
    local equippedItem = FormatEquipmentItem(item, equipmentSlot, Player)
    if not equippedItem then
        return { success = false, message = 'This item cannot be equipped' }
    end

    if not RemoveItem(src, item.name, 1, itemSlot, 'equipped ' .. equipmentSlot) then
    ...
    elseif equipmentSlot == 'simcard' and SetActivePhoneNumber then
        local phoneNumber = equippedItem.info and equippedItem.info.simNumber
        if phoneNumber then SetActivePhoneNumber(Player, phoneNumber) end
    elseif (equipmentSlot == 'gadget' or equipmentSlot == 'gadget2') and equippedItem.name == 'parachute' then
        TriggerClientEvent('prp-inventory:client:equipParachute', src)
    end
    ...
    return {
        ...
        itemName = item.name,
        message = ('%s equipped'):format(equippedItem.label or EquipmentLabels[equipmentSlot] or 'Item')
    }
end

local function UnequipEquipmentItem(src, equipmentSlot, targetSlot)
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return { success = false, message = 'Player not found' } end
    ...
    if targetSlot then
        if targetSlot < 1 or targetSlot > Config.MaxSlots then
            return { success = false, message = 'Invalid inventory slot' }
        end

        for _, targetItem in pairs(Player.PlayerData.items or {}) do
            if targetItem and tonumber(targetItem.slot) == targetSlot then
                return { success = false, message = 'That inventory slot is occupied' }
            end
        end
    end

    local info = CloneTable(equippedItem.info)
    if not AddItem(src, equippedItem.name, 1, targetSlot or false, info, 'unequipped item') then
    ...
    elseif equipmentSlot == 'simcard' and ClearActivePhoneNumber then
        ClearActivePhoneNumber(Player, info.simNumber)
    elseif (equipmentSlot == 'gadget' or equipmentSlot == 'gadget2') and equippedItem.name == 'parachute' then
        TriggerClientEvent('prp-inventory:client:removeParachute', src)
    end
...
local function ApplyArmorPlate(src, plateSlot)
    ...
    local currentPlates = Clamp(info.plates, 0, ArmorPlateCapacity)
    if currentPlates >= ArmorPlateCapacity then
        return { success = false, message = 'Armour plate storage is full' }
    end
    ...
    info.maxArmor = 100
    info.plateCapacity = ArmorPlateCapacity
    info.plates = Clamp(currentPlates + 1, 0, ArmorPlateCapacity)
    info.description = ('Armor: %d%% (%d/%d plates)'):format(newArmor, info.plates, ArmorPlateCapacity)
    armour.info = info
    armour.description = info.description
    equipment.armour = armour
...
exports('EquipEquipmentItem', function(source, equipmentSlot, slot)
    return EquipEquipmentItem(source, equipmentSlot, slot)
end)
...
function GetEquippedBackpackInventoryPayload(Player)
    local backpack, info = EnsureEquippedBackpackInfo(Player)
    if not backpack or not info or not info.storageId then return nil end

    local inventory = GetOrCreatePlayerBackpackInventory(info.storageId, {
        label = backpack.label or 'Backpack',
        slots = info.slots,
        maxweight = info.maxweight
    }, Player)

    inventory.maxweight = info.maxweight
    inventory.slots = info.slots
    inventory.label = backpack.label or 'Backpack'
    inventory.isOpen = false -- embedded backpack must not block/reopen itself
    ...
    return {
        ...
        maxweight = info.maxweight,
        slots = info.slots,
        inventory = PackInventoryItemsForStorage(inventory.items)
    }
end

local function SendEmbeddedBackpackInventory(src, storageId, backpack, info, Player, playerItemsOverride)
    local backpackPayload = GetEquippedBackpackInventoryPayload(Player)
    if not backpackPayload then return false end

    TriggerClientEvent('qb-inventory:client:updateInventory', src, {
        inventory = playerItemsOverride or Player.PlayerData.items,
        equipment = Player.PlayerData.metadata and Player.PlayerData.metadata.equipment or {},
        quickslots = Player.PlayerData.metadata and Player.PlayerData.metadata.quickslots or {},
...
local function OpenEquippedBackpack(src)
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return { success = false, message = 'Player not found' } end

    local equipment = GetEquipment(Player)
    local backpack = equipment.backpack
    if not backpack or not backpack.name then
        return { success = false, message = 'No backpack equipped' }
    end

    local info = CloneTable(backpack.info)
    local changed = false

    if not info.storageId then
        info.storageId = ('backpack_%s_%s%s'):format(Player.PlayerData.citizenid, QBCore.Shared.RandomStr(3), QBCore.Shared.RandomInt(4))
        changed = true
    end
    ...
    SendEmbeddedBackpackInventory(src, info.storageId, backpack, info, Player)
    return { success = true }
end

exports('OpenEquippedBackpack', OpenEquippedBackpack)

RegisterNetEvent('prp-inventory:server:openBackpack', function()
    local src = source
    local result = OpenEquippedBackpack(src)
    if result and not result.success and result.message then
        TriggerClientEvent('QBCore:Notify', src, result.message, 'error')
    end
end)

RegisterNetEvent('qb-inventory:server:openBackpack', function()
    local src = source
    local result = OpenEquippedBackpack(src)
    if result and not result.success and result.message then
        TriggerClientEvent('QBCore:Notify', src, result.message, 'error')
...
AddEventHandler('playerDropped', function()
    for identifier, inv in pairs(Inventories) do
        if inv.isOpen == source then
            if inv.storageKind == 'player_backpack' and inv.ownerCitizenid then
                SaveBackpackItemsByCitizenId(inv.ownerCitizenid, identifier, inv.items or {})
            end
            inv.isOpen = false
        end
    end
...
AddEventHandler('txAdmin:events:serverShuttingDown', function()
    for inventory, data in pairs(Inventories) do
        if data.isOpen then
            if IsBackpackInventoryId(inventory) and data.ownerCitizenid then
                SaveBackpackItemsByCitizenId(data.ownerCitizenid, inventory, data.items or {})
            else
...
local function ParseWeaponTintItem(itemName)
    itemName = tostring(itemName or '')
    local mk2Tint = itemName:match('^weapontint_mk2_(%d+)$')
    if mk2Tint then return tonumber(mk2Tint), true end

    local tint = itemName:match('^weapontint_(%d+)$')
    if tint then return tonumber(tint), false end

    return nil, false
end

local function IsMk2WeaponName(weaponName)
    return tostring(weaponName or ''):find('_mk2', 1, true) ~= nil
end
...
QBCore.Functions.CreateCallback('qb-inventory:server:unequipItem', function(source, cb, equipmentSlot, targetSlot)
    local result = UnequipEquipmentItem(source, equipmentSlot, targetSlot)
    if result and not result.success and result.message then
        TriggerClientEvent('QBCore:Notify', source, result.message, 'error')
    end
    cb(result)
...
local function SetPlayerQuickSlot(source, quickSlot, itemSlot)
    local Player = QBCore.Functions.GetPlayer(source)
    if not Player then
        return { success = false, message = 'Player not found' }
    end

    if type(Player.PlayerData.metadata) ~= 'table' then
    ...
    if not quickSlot or quickSlot < 1 or quickSlot > QuickSlotCount then
        return { success = false, message = 'Invalid quick slot' }
    end
    ...
    if itemSlot then
        local item = FindPlayerInventoryItemBySlot(Player, itemSlot)
        if not item then
            return { success = false, message = 'No item in that inventory slot' }
        end
        for i = 1, QuickSlotCount do
            local assigned = tonumber(quickslots[tostring(i)] or quickslots[i])
            if assigned == itemSlot then
    ...
    return {
        ...
        equipment = Player.PlayerData.metadata.equipment or {}
    }
end

QBCore.Functions.CreateCallback('qb-inventory:server:setQuickSlot', function(source, cb, quickSlot, itemSlot)
    cb(SetPlayerQuickSlot(source, quickSlot, itemSlot))
end)

RegisterNetEvent('qb-inventory:server:setQuickSlotDirect', function(quickSlot, itemSlot)
    local src = source
    local result = SetPlayerQuickSlot(src, quickSlot, itemSlot)
    if result and not result.success and result.message then
        TriggerClientEvent('QBCore:Notify', src, result.message, 'error')
    end
end)
...
local function GenerateUniqueSimNumber()
    for _ = 1, 25 do
        ...
        if not existsOnline and not existsSaved and not existsInventory and not existsEquipment then return number end
    end
...
SetActivePhoneNumber = function(Player, phoneNumber)
...
ClearActivePhoneNumber = function(Player, phoneNumber)
    if not Player then return end
...
local function ApplyDeviceAttachment(src, deviceSlot, attachmentSlot)
    ...
    if PhoneItems[mode] then
        ...
        inventory = Player.PlayerData.items or {}
        deviceItem = GetInventoryItemBySlot(inventory, deviceSlot, mode)
        if not deviceItem or not deviceItem.name or not PhoneItems[deviceItem.name:lower()] then
...
local function RemoveDeviceAttachment(src, deviceSlot, attachmentName, preferredSlot)
    ...
    if PhoneItems[deviceName] and attachmentName == 'simcard' then
...
QBCore.Functions.CreateCallback('qb-inventory:server:applyWeaponAttachment', function(source, cb, weaponData, attachmentData)
    ...
    local tintIndex, mk2Tint = ParseWeaponTintItem(attachmentName)
    if tintIndex then
        if mk2Tint ~= IsMk2WeaponName(weaponItem.name) then
            cb({ success = false, message = mk2Tint and 'This skin only fits MK2 weapons' or 'This skin only fits standard weapons' })
            return
        end

        weaponItem.info = type(weaponItem.info) == 'table' and weaponItem.info or {}
        if tonumber(weaponItem.info.tint) == tintIndex then
            cb({ success = false, message = 'This skin is already installed' })
            return
        end

        if not RemoveItem(src, attachmentName, 1, attachmentSlot, 'weapon skin applied') then
            cb({ success = false, message = 'Could not remove skin item' })
            return
        end
        ...
        weaponItem.info = type(weaponItem.info) == 'table' and weaponItem.info or {}
        weaponItem.info.tint = tintIndex
        weaponItem.info.tintItem = attachmentName
...
QBCore.Functions.CreateCallback('qb-inventory:server:removeWeaponAttachment', function(source, cb, weaponData, attachmentData)
    ...
    local tintIndex = ParseWeaponTintItem(attachmentName)
    if attachmentData.isTint or tintIndex then
        if type(weaponItem.info) ~= 'table' or weaponItem.info.tint == nil then
            cb({ success = false, message = 'This skin is not installed' })
            return
        end

        local returnItem = weaponItem.info.tintItem or attachmentName
        if not AddItem(src, returnItem, 1, false, false, 'weapon skin removed') then
            cb({ success = false, message = 'No space for removed skin' })
            return
        end

        weaponItem.info.tint = nil
        weaponItem.info.tintItem = nil
        local _, weaponKey = GetInventoryItemBySlot(inventory, weaponSlot, weaponData.name)
        inventory[weaponKey or weaponSlot] = weaponItem
        Player.Functions.SetPlayerData('items', inventory)

        TriggerClientEvent('qb-inventory:client:ItemBox', src, QBCore.Shared.Items[returnItem], 'add')
        TriggerClientEvent('qb-inventory:client:updateInventory', src)
...
RegisterNetEvent('qb-inventory:server:closeInventory', function(inventory)
    ...
    if IsBackpackInventoryId(inventory) then
        SaveSingleInventory(inventory, QBPlayer)
        return
    end
...
IsBackpackInventoryId = function(inventoryId)
    return type(inventoryId) == 'string' and inventoryId:find('backpack_') == 1
end

local function EnsureBackpackInventoryForMove(inventoryId, src)
    if not IsBackpackInventoryId(inventoryId) then return end

    local Player = src and QBCore.Functions.GetPlayer(src) or nil
    GetOrCreatePlayerBackpackInventory(inventoryId, {
        label = 'Backpack',
        slots = tonumber(Config.BackpackStorage and Config.BackpackStorage.slots) or 20,
        maxweight = tonumber(Config.BackpackStorage and Config.BackpackStorage.maxweight) or 50000
    }, Player)
end
...
local function GetInventoryMaxWeight(inventoryId, src)
    if inventoryId == 'player' then
        return Config.MaxWeight
    elseif type(inventoryId) == 'string' and inventoryId:find('otherplayer%-') then
        return Config.MaxWeight
    elseif type(inventoryId) == 'string' and inventoryId:find('drop%-') == 1 then
        return Drops[inventoryId] and Drops[inventoryId].maxweight or nil
    elseif Inventories[inventoryId] then
        return Inventories[inventoryId].maxweight
    end
...
local function GetInventoryWeight(items)
    ...
    for _, item in pairs(items or {}) do
        if item and item.name then
            ...
            local itemWeight = tonumber(item.weight) or (itemInfo and tonumber(itemInfo.weight)) or 0
            weight = weight + (itemWeight * (tonumber(item.amount) or 1))
        end
    end
    return weight
end

local function GetMovedItemWeight(item, amount)
    if not item or not item.name then return 0 end

    local itemInfo = QBCore.Shared.Items[tostring(item.name):lower()]
    local weight = tonumber(item.weight) or (itemInfo and tonumber(itemInfo.weight)) or 0
    return weight * (tonumber(amount) or tonumber(item.amount) or 1)
end

local function CanInventoryAcceptWeight(inventoryId, src, items, addedWeight)
    addedWeight = tonumber(addedWeight) or 0
    if addedWeight <= 0 then return true end

    local maxWeight = tonumber(GetInventoryMaxWeight(inventoryId, src))
    if not maxWeight or maxWeight <= 0 then return true end

    return GetInventoryWeight(items) + addedWeight <= maxWeight
end

local function PutInventoryItemInSlot(items, slot, item)
    slot = tonumber(slot)
    if not slot or type(items) ~= 'table' then return end

    local existingKey = getItemKey(items, slot)
    if existingKey then
        items[existingKey] = nil
    end
...
local function MoveInventorySlotDirect(src, fromInventory, toInventory, fromSlot, toSlot, fromAmount)
    if fromInventory == toInventory and tonumber(fromSlot) == tonumber(toSlot) then
        return false
    end

    EnsureBackpackInventoryForMove(fromInventory, src)
    EnsureBackpackInventoryForMove(toInventory, src)

    local fromItems = getInventoryItems(fromInventory, src)
    local toItems = fromInventory == toInventory and fromItems or getInventoryItems(toInventory, src)
    local fromKey = getItemKey(fromItems, fromSlot)
    if not fromKey then return false end

    local sourceItem = fromItems[fromKey]
    if not sourceItem or not sourceItem.name then return false end

    local sourceAmount = tonumber(sourceItem.amount) or 1
    local moveAmount = math.min(tonumber(fromAmount) or sourceAmount, sourceAmount)
    if moveAmount < 1 then return false end

    local toKey = getItemKey(toItems, toSlot)
    local targetItem = toKey and toItems[toKey] or nil

    if targetItem and targetItem.name == sourceItem.name and CanStackInventoryItems(sourceItem, targetItem) then
        if fromInventory ~= toInventory and not CanInventoryAcceptWeight(toInventory, src, toItems, GetMovedItemWeight(sourceItem, moveAmount)) then
            return false, 'Not enough weight available'
        end

        targetItem.amount = (tonumber(targetItem.amount) or 1) + moveAmount
        sourceItem.amount = sourceAmount - moveAmount
        if sourceItem.amount <= 0 then
            fromItems[fromKey] = nil
        end

    elseif targetItem then
        local targetAmount = tonumber(targetItem.amount) or 1
        local sourceWeight = GetMovedItemWeight(sourceItem, sourceAmount)
        local targetWeight = GetMovedItemWeight(targetItem, targetAmount)

        if fromInventory ~= toInventory then
            local fromMaxWeight = tonumber(GetInventoryMaxWeight(fromInventory, src))
            local toMaxWeight = tonumber(GetInventoryMaxWeight(toInventory, src))
            local fromWeightAfter = GetInventoryWeight(fromItems) - sourceWeight + targetWeight
            local toWeightAfter = GetInventoryWeight(toItems) - targetWeight + sourceWeight

            if fromMaxWeight and fromMaxWeight > 0 and fromWeightAfter > fromMaxWeight then
                return false, 'Not enough weight available'
            end

            if toMaxWeight and toMaxWeight > 0 and toWeightAfter > toMaxWeight then
                return false, 'Not enough weight available'
            end
        end

        local movingItem = CloneTable(sourceItem)
        local swappedItem = CloneTable(targetItem)
        fromItems[fromKey] = nil
        toItems[toKey] = nil
        PutInventoryItemInSlot(toItems, toSlot, movingItem)
        PutInventoryItemInSlot(fromItems, fromSlot, swappedItem)
        moveAmount = sourceAmount
        ...
        local movingItem = CloneTable(sourceItem)
        movingItem.amount = moveAmount

        if moveAmount >= sourceAmount then
            fromItems[fromKey] = nil
        else
            sourceItem.amount = sourceAmount - moveAmount
        end
    ...
    if fromInventory == 'player' then
        playerItems = fromItems
    elseif toInventory == 'player' then
        playerItems = toItems
    end

    return true, moveAmount, playerItems
end
...
SaveSingleInventory = function(identifier, Player)
    if not identifier or not Inventories or not Inventories[identifier] then return end

    if IsBackpackInventoryId(identifier) then
        local saved = SavePlayerBackpackItems(Player, identifier, Inventories[identifier].items or {})
        if saved then return end
    end

    MySQL.prepare('INSERT INTO inventories (identifier, items) VALUES (?, ?) ON DUPLICATE KEY UPDATE items = ?', {
        identifier,
        json.encode(Inventories[identifier].items or {}),
        json.encode(Inventories[identifier].items or {})
    })
end

local function RefreshEquippedBackpackInventory(src, Player, playerItemsOverride)
    local equipment = Player and GetEquipment(Player)
    local backpack = equipment and equipment.backpack
    if not backpack or not backpack.info or not backpack.info.storageId then return false end

    local info = CloneTable(backpack.info)
    info.slots = tonumber(info.slots) or tonumber(Config.BackpackStorage and Config.BackpackStorage.slots) or 20
    info.maxweight = tonumber(info.maxweight or info.maxWeight) or tonumber(Config.BackpackStorage and Config.BackpackStorage.maxweight) or 50000
    SendEmbeddedBackpackInventory(src, info.storageId, backpack, info, Player, playerItemsOverride)
    return true
end
...
RegisterNetEvent('qb-inventory:server:SetInventoryData', function(fromInventory, toInventory, fromSlot, toSlot, fromAmount, toAmount)
    ...
    fromInventory = ResolveBackpackInventoryAlias(Player, fromInventory)
    toInventory = ResolveBackpackInventoryAlias(Player, toInventory)
    ...
    local involvesBackpackInventory = IsBackpackInventoryId(fromInventory) or IsBackpackInventoryId(toInventory)
    local movedPlayerItems
    ...
    if involvesBackpackInventory then
        local moved, result, playerItems = MoveInventorySlotDirect(src, fromInventory, toInventory, fromSlot, toSlot, fromAmount)
        if not moved then
            if type(result) == 'string' then
                TriggerClientEvent('QBCore:Notify', src, result, 'error')
            end
            if not RefreshEquippedBackpackInventory(src, Player) then
                TriggerClientEvent('qb-inventory:client:updateInventory', src)
            end
            return
        end

        fromAmount = result or fromAmount
        movedPlayerItems = playerItems
        inventoryChanged = true

    elseif toItem and fromItem.name == toItem.name then
    ...
    if inventoryChanged then
        if type(fromInventory) == 'string' and fromInventory:find('backpack_') == 1 then
            SaveSingleInventory(fromInventory, Player)
        end

        if type(toInventory) == 'string' and toInventory:find('backpack_') == 1 then
            SaveSingleInventory(toInventory, Player)
        end
    end
    ...
    if inventoryChanged and involvesBackpackInventory then
        RefreshEquippedBackpackInventory(src, Player, movedPlayerItems)
    else
        TriggerClientEvent('qb-inventory:client:updateInventory', src)
    end
