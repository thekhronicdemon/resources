...
local function GiveStarterItems(source)
    ...
    for _, v in pairs(QBCore.Shared.StarterItems) do
        ...
        exports['prp-inventory']:AddItem(src, v.item, v.amount, false, info, 'prp-multicharacters:GiveStarterItems')
    end
end
...
QBCore.Commands.Add('logout', Lang:t('commands.logout_description'), {}, false, function(source)
    ...
    TriggerClientEvent('prp-multicharacters:client:chooseChar', src)
end, 'admin')

QBCore.Commands.Add('closeNUI', Lang:t('commands.closeNUI_description'), {}, false, function(source)
    local src = source
    TriggerClientEvent('prp-multicharacters:client:closeNUI', src)
end)
...
RegisterNetEvent('prp-multicharacters:server:disconnect', function()
    local src = source
    DropPlayer(src, Lang:t('commands.droppedplayer'))
end)

RegisterNetEvent('prp-multicharacters:server:loadUserData', function(cData)
    local src = source
    if QBCore.Player.Login(src, cData.citizenid) then
        ...
        if Config.SkipSelection then
            ...
            TriggerClientEvent('prp-multicharacters:client:spawnLastLocation', src, coords, cData)
        else
...
RegisterNetEvent('prp-multicharacters:server:createCharacter', function(data)
    local src = source
    local newData = {}
    ...
    if QBCore.Player.Login(src, false, newData) then
        ...
        if GetResourceState('qb-apartments') == 'started' and Apartments.Starting then
            ...
            QBCore.Commands.Refresh(src)
            loadHouseData(src)
            TriggerClientEvent('prp-multicharacters:client:closeNUIdefault', src)
            GiveStarterItems(src)
...
RegisterNetEvent('prp-multicharacters:server:deleteCharacter', function(citizenid)
    local src = source
...
QBCore.Functions.CreateCallback('prp-multicharacters:server:GetUserCharacters', function(source, cb)
    local src = source
...
QBCore.Functions.CreateCallback('prp-multicharacters:server:GetServerLogs', function(_, cb)
    MySQL.query('SELECT * FROM server_logs', {}, function(result)
        cb(result)
    end)
end)

QBCore.Functions.CreateCallback('prp-multicharacters:server:GetNumberOfCharacters', function(source, cb)
    local src = source
    local license = QBCore.Functions.GetIdentifier(src, 'license')
    local numOfChars = 0
...
QBCore.Functions.CreateCallback('prp-multicharacters:server:setupCharacters', function(source, cb)
...
QBCore.Functions.CreateCallback('prp-multicharacters:server:getSkin', function(_, cb, cid)
