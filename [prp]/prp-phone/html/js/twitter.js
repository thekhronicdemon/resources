...
$(document).on('click', '.twitter-header-tab', function(e){
    ...
    if (PressedTwitterTab !== CurrentTwitterTab) {
        ...
        if (PressedTwitterTab === "twitter-mentions") {
            $.post('https://prp-phone/ClearMentions');
        }
...
$(document).on('click', '.twitter-new-tweet', function(e){
    ...
    PRP.Phone.Animations.TopSlideDown(".twitter-new-tweet-tab", 450, 0);
});
...
PRP.Phone.Notifications.LoadTweets = function(Tweets) {
    ...
    if (Tweets !== null && Tweets !== undefined && Tweets !== "" && Tweets.length > 0) {
        ...
        $.each(Tweets, function(i, Tweet){
            ...
            var TwtMessage = PRP.Phone.Functions.FormatTwitterMessage(clean);
            ...
            // if (Tweet.citizenid === PRP.Phone.Data.PlayerData.citizenid){
...
$(document).on('click','#twt-delete-click',function(e){
    ...
    $.post('https://prp-phone/DeleteTweet', JSON.stringify({id: source}))
})
...
PRP.Phone.Notifications.LoadMentionedTweets = function(Tweets) {
    Tweets = Tweets.reverse();
    $('#tweet-new-url').val("");
    if (Tweets.length > 0) {
...
PRP.Phone.Functions.FormatTwitterMessage = function(TwitterMessage) {
...
$(document).on('click', '#send-tweet', function(e){
    ...
    if (TweetMessage != "") {
        ...
        $.post('https://prp-phone/PostNewTweet', JSON.stringify({
            Message: TweetMessage,
            Date: CurrentDate,
            Picture: PRP.Phone.Data.MetaData.profilepicture,
            url: imageURL
        }), function(Tweets){
            PRP.Phone.Notifications.LoadTweets(Tweets);
        });
        ...
        PRP.Phone.Animations.TopSlideUp(".twitter-new-tweet-tab", 450, -120);
    } else {
        PRP.Phone.Notifications.Add("fas fa-comments", "Chatterly", "Fill a message!", "#2f80ed");
    };
    $('#tweet-new-url').val("");
...
$(document).on('click', '.hashtag-tag-text', function(e){
    ...
    if (!HashtagOpen) {
        ...
        $.post('https://prp-phone/GetHashtagMessages', JSON.stringify({hashtag: Hashtag}), function(HashtagData){
            PRP.Phone.Notifications.LoadHashtagMessages(HashtagData.messages);
        });
...
function CopyMentionTag(elem) {
    ...
    PRP.Phone.Notifications.Add("fas fa-comments", "Chatterly", $(elem).data('mentiontag')+ " copied!", "#2f80ed", 1250);
    document.execCommand("copy");
    $temp.remove();
}

PRP.Phone.Notifications.LoadHashtags = function(hashtags) {
    if (hashtags !== null) {
        $(".twitter-hashtags").html("");

        $.each(hashtags, function(i, hashtag){
            var Elem = '';
            var TweetHandle = "Chatter";
            if (hashtag.messages.length > 1 ) {
               TweetHandle = "Chatters";
            }
...
PRP.Phone.Notifications.LoadHashtagMessages = function(Tweets) {
    Tweets = Tweets.reverse();
    if (Tweets !== null && Tweets !== undefined && Tweets !== "" && Tweets.length > 0) {
...
$(document).on('click', '.twitter-hashtag', function(event){
    ...
    PRP.Phone.Notifications.LoadHashtagMessages(TweetData.messages);
