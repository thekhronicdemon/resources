...
SetupMeosHome = function() {
    $("#meos-app-name").html("Welcome " + PRP.Phone.Data.PlayerData.charinfo.firstname + " " + PRP.Phone.Data.PlayerData.charinfo.lastname);
}
...
$(document).on('click', '.house-adress-location', function(e){
    ...
    $.post('https://prp-phone/SetGPSLocation', JSON.stringify({
...
$(document).on('click', '.appartment-adress-location', function(e){
    ...
    $.post('https://prp-phone/SetApartmentLocation', JSON.stringify({
...
$(document).on('click', '.person-search-result-apartment > span', function(e){
    ...
    PRP.Phone.Notifications.Add("fas fa-university", "MDT", "House number copied!", "#badc58", 1750);
    ...
    if (SearchName !== "") {
        $.post('https://prp-phone/FetchSearchResults', JSON.stringify({
            ...
            if (result != null) {
                ...
                PRP.Phone.Notifications.Add("politie", "MDT", "There are no search results!");
                $(".person-search-results").html("");
            }
        });
    } else {
...
$(document).on('click', '.confirm-search-person-house', function(e){
    ...
    if (SearchName !== "") {
        $.post('https://prp-phone/FetchPlayerHouses', JSON.stringify({
            input: SearchName,
        }), function(result){
            if (result != null) {
                $(".person-search-results").html("");
...
$(document).on('click', '.confirm-search-vehicle', function(e){
    ...
    if (SearchName !== "") {
        $.post('https://prp-phone/FetchVehicleResults', JSON.stringify({
            input: SearchName,
        }), function(result){
            if (result != null) {
                $(".vehicle-search-results").html("");
...
$(document).on('click', '.scan-search-vehicle', function(e){
    e.preventDefault();
    $.post('https://prp-phone/FetchVehicleScan', JSON.stringify({}), function(vehicle){
        if (vehicle != null) {
            $(".vehicle-search-results").html("");
            var APK = "Yes";
            if (!vehicle.status) {
            ...
            PRP.Phone.Notifications.Add("politie", "MDT", "No vehicle nearby!");
...
$(document).on('click', '.meos-recent-alert', function(e){
    ...
    if (alertData != undefined){
        if (alertData.coords != undefined && alertData.coords != null) {
            $.post('https://prp-phone/SetAlertWaypoint', JSON.stringify({
                alert: alertData,
            }));
        } else {
            PRP.Phone.Notifications.Add("politie", "MDT", "This alert doesn't have a GPS location!");
        }
    }else {
        PRP.Phone.Notifications.Add("politie", "MDT", "There are no alerts available.");
    }
});
...
$(document).on('click', '.meos-clear-alerts', function(e){
    ...
    PRP.Phone.Notifications.Add("politie", "MDT", "All alerts have been deleted!");
});
