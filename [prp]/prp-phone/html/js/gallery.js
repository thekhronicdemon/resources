...
$(document).on('click', '#delete-button', function(e){
    ...
    setTimeout(() => {
        $.post('https://prp-phone/DeleteImage', JSON.stringify({image:source}), function(Hashtags){
            setTimeout(()=>{
                $('#return-button').click()
...
$(document).on('click', '#tweet-button', function(e){
    ...
    if (TweetMessage != "") {
        ...
        PRP.Phone.Notifications.Add("fab fa-twitter", "Twitter", "Fill a message!", "#1DA1F2");
    };
    $('#tweet-new-url').val("");
...
$(document).on('click', '#advert-button', function(e){
    ...
    if (Advert !== "") {
        ...
        if (!picture){
            $.post('https://prp-phone/PostAdvert', JSON.stringify({
                message: Advert,
                url: null
            }));
            returnDetail()
