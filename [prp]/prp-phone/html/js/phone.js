...
$(document).on('click', '.phone-app-footer-button', function(e){
    ...
    if (PressedFooterTab !== CurrentFooterTab) {
        ...
        if (PressedFooterTab == "recent") {
            $.post('https://prp-phone/ClearRecentAlerts');
...
PRP.Phone.Functions.SetupRecentCalls = function(recentcalls) {
...
$(document).on('click', '.phone-recent-call', function(e){
    ...
    $.post('https://prp-phone/CallContact', JSON.stringify({
        ContactData: cData,
...
PRP.Phone.Functions.LoadContacts = function(myContacts) {
...
$(document).on('click', '#new-chat-phone', function(e){
    ...
    if (ContactData.number !== PRP.Phone.Data.PlayerData.charinfo.phone) {
        ...
        setTimeout(function(){
            ...
            PRP.Phone.Functions.ToggleApp("phone", "none");
            PRP.Phone.Functions.ToggleApp("whatsapp", "block");
            PRP.Phone.Data.currentApplication = "whatsapp";
        ...
        PRP.Phone.Notifications.Add("fa fa-phone-alt", "Phone", "You can't whatsapp yourself..", "default", 3500);
    }
});
...
$(document).on('click', '#edit-contact', function(e){
    ...
    PRP.Phone.Animations.TopSlideDown(".phone-edit-contact", 200, 0);
});
...
$(document).on('click', '#edit-contact-save', function(e){
    ...
    if (ContactName != "" && ContactNumber != "") {
        $.post('https://prp-phone/EditContact', JSON.stringify({
            ...
            PRP.Phone.Functions.LoadContacts(PhoneContacts);
        });
        PRP.Phone.Animations.TopSlideUp(".phone-edit-contact", 250, -100);
        setTimeout(function(){
            $(".phone-edit-contact-number").val("");
            $(".phone-edit-contact-name").val("");
        }, 250)
    } else {
        PRP.Phone.Notifications.Add("fas fa-exclamation-circle", "Edit Contact", "Fill out all fields!");
    }
});
...
$(document).on('click', '#edit-contact-delete', function(e){
    ...
    $.post('https://prp-phone/DeleteContact', JSON.stringify({
        CurrentContactName: ContactName,
        CurrentContactNumber: ContactNumber,
        CurrentContactIban: ContactIban,
    }), function(PhoneContacts){
...
$(document).on('click', '#phone-plus-icon', function(e){
    ...
    PRP.Phone.Animations.TopSlideDown(".phone-add-contact", 200, 0);
});
...
$(document).on('click', '#add-contact-save', function(e){
    ...
    if (ContactName != "" && ContactNumber != "") {
        $.post('https://prp-phone/AddNewContact', JSON.stringify({
        ...
        PRP.Phone.Animations.TopSlideUp(".phone-add-contact", 250, -100);
        setTimeout(function(){
        ...
        if (SelectedSuggestion !== null) {
            $.post('https://prp-phone/RemoveSuggestion', JSON.stringify({
        ...
        PRP.Phone.Notifications.Add("fas fa-exclamation-circle", "Add Contact", "Fill out all fields!");
    }
});
...
CancelOutgoingCall = function() {
    if (PRP.Phone.Data.currentApplication == "phone-call") {
        PRP.Phone.Animations.TopSlideUp('.phone-application-container', 400, -160);
...
$(document).on('click', '#outgoing-cancel', function(e){
    ...
    $.post('https://prp-phone/CancelOutgoingCall');
});

$(document).on('click', '#incoming-deny', function(e){
    e.preventDefault();

    $.post('https://prp-phone/DenyIncomingCall');
});

$(document).on('click', '#ongoing-cancel', function(e){
    e.preventDefault();

    $.post('https://prp-phone/CancelOngoingCall');
});

PRP.Phone.Functions.SetSpeakerPhone = function(enabled) {
    $("#ongoing-speaker").toggleClass("speaker-active", !!enabled);
}

$(document).on('click', '#ongoing-speaker', function(e){
    e.preventDefault();

    var enabled = !$(this).hasClass("speaker-active");
    $.post('https://prp-phone/ToggleSpeakerPhone', JSON.stringify({
        enabled: enabled,
    }), function(resp){
        PRP.Phone.Functions.SetSpeakerPhone(resp && resp.enabled);
    });
});

IncomingCallAlert = function(CallData, Canceled, AnonymousCall) {
    if (!Canceled) {
        if (!PRP.Phone.Data.CallActive) {
            PRP.Phone.Animations.TopSlideUp('.phone-application-container', 400, -160);
            PRP.Phone.Animations.TopSlideUp('.'+PRP.Phone.Data.currentApplication+"-app", 400, -160);
            setTimeout(function(){
                ...
                $(".phone-app").css({"display":"none"});
                PRP.Phone.Functions.HeaderTextColor("white", 400);
                $("."+PRP.Phone.Data.currentApplication+"-app").css({"display":"none"});
            ...
            PRP.Phone.Data.currentApplication = "phone-call";
            PRP.Phone.Data.CallActive = true;
        }
        setTimeout(function(){
...
PRP.Phone.Functions.SetupCurrentCall = function(cData) {
    if (cData.InCall) {
        CallData = cData;
...
$(document).on('click', '.phone-currentcall-container', function(e){
    ...
    PRP.Phone.Functions.HeaderTextColor("white", 500);
    PRP.Phone.Animations.TopSlideDown('.phone-application-container', 500, 0);
    PRP.Phone.Animations.TopSlideDown('.phone-call-app', 500, 0);
    PRP.Phone.Functions.ToggleApp("phone-call", "block");
...
$(document).on('click', '#incoming-answer', function(e){
    ...
    $.post('https://prp-phone/AnswerCall');
});

PRP.Phone.Functions.AnswerCall = function(CallData) {
    $(".phone-call-incoming").css({"display":"none"});
    $(".phone-call-outgoing").css({"display":"none"});
    $(".phone-call-ongoing").css({"display":"block"});
...
PRP.Phone.Functions.SetupSuggestedContacts = function(Suggested) {
